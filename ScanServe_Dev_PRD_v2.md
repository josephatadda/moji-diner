# ScanServe — Development PRD & User Flow Documentation
...
[Owner clicks "Download All QR Codes"]
  ...
  → Each QR includes: restaurant name, table number, ScanServe branding
...
const generateTableQR = async (slug: string, tableNumber: number) => {
  const url = `https://scanserve.ng/${slug}/t/${tableNumber}`
...
const getDinerSession = (): string => {
  const key = 'scanserve_session_id'
...
const initializePayment = async (order: Order) => {
  ...
  const handler = PaystackPop.setup({
    ...
    email: `table-${order.table_number}@${order.restaurant_slug}.scanserve.ng`,
...
[Parts 2–4 share link: scanserve.ng/split/XK9P2R/2]
        │
        ▼
...
> **V1 inclusion rationale:** Loyalty is the primary mechanism for capturing diner phone numbers, enabling repeat marketing, and differentiating ScanServe from a generic payment tool. Building it in V1 means every early order builds a customer database the restaurant actually owns.
  ...
  runtimeCaching: [
    {
      ...
      urlPattern: /^https:\/\/scanserve\.ng\/api\/menu\/.*/,
...
const DB_NAME = 'scanserve-offline'
...
*ScanServe Development PRD v2.0 — Internal Document*  
*Scan. Order. Pay. Done.*
